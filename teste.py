...
OPENAI_API_KEY = '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'
...
class PDF(FPDF):
    ...
    def tabela_reprovados(self, dados):
        ...
        # Cabeçalho: apenas duas colunas
        self.cell(120, 8, 'FORNECEDOR', border=1, align='C', fill=True)
        self.cell(25, 8, 'NOTA IQF', border=1, ln=1, align='C', fill=True)

        self.set_font('Helvetica', '', 10)
        fill = False
        row_height = 6

        for nome, nota, _ in dados:  # documento ignorado com "_"
        ...
        # Nome do fornecedor completo em maiúsculo
            nome_upper = nome.upper()

        # Alternar cor de fundo
            self.set_fill_color(245, 245, 245) if fill else self.set_fill_color(255, 255, 255)
            self.set_text_color(0, 0, 0)
            self.cell(120, row_height, nome_upper[:50], border=1, align='C', fill=fill)

         # Cor vermelha se IQF < 70
            if nota < 70:
                self.set_text_color(255, 0, 0)
            else:
                self.set_text_color(0, 0, 0)

            self.cell(25, row_height, f"{nota:.2f}", border=1, ln=1, align='C', fill=fill)
            fill = not fill
...
async def gerar_feedback_individual(update, context, nome_fornecedor, mes):
    df = carregar_dados_qualidade()
    ...
    # Filtra os dados pelo mês e fornecedor
    df_mes = df[(df["mes"] == mes) & (df["nome_agente"] == nome_fornecedor)]

    if df_mes.empty:
        return f"❌ Dados não encontrados para {nome_fornecedor} em {mes}."

    # Calcula a média das notas para esse fornecedor no mês
    nota = df_mes["nota"].mean()

    # Garantindo que a classificação seja feita corretamente com base na média de nota
    if nota > 75:
        return f"📊 Nota IQF: {nota:.2f}\n🏆 Classificação: Aprovado – Desempenho de excelência"
    elif 70 <= nota <= 75:
        return f"📊 Nota IQF: {nota:.2f}\n⚠️ Classificação: Em Atenção – Performance no limite mínimo"
    else:
        return f"📊 Nota IQF: {nota:.2f}\n❌ Classificação: Reprovado – Abaixo do padrão mínimo (70,00)"
...
async def handle_analise_mensal(update, context, mes):
    ...
    # Garantir que as colunas de nota sejam numéricas e remover valores nulos
    df_qualidade['nota'] = pd.to_numeric(df_qualidade['nota'], errors='coerce')
    df_qualidade['data'] = pd.to_datetime(df_qualidade['data'], errors='coerce')
    df_qualidade['mes'] = df_qualidade['data'].dt.strftime('%m/%Y')
    df_qualidade['nome_agente'] = df_qualidade['nome_agente'].astype(str).str.strip().str.upper()

    df_qualidade = df_qualidade.dropna(subset=['nota', 'mes'])
    ...
    # Calcular a média global para o mês
    media_global = round(dfm['nota'].mean(), 2)

    # Calcular a média individual de cada fornecedor
    media_mes = (
        dfm.groupby('nome_agente')['nota']
        .mean()
        .reset_index()
        .rename(columns={'nome_agente': 'fornecedor', 'nota': 'media'})
    )
    
    # Arredondar a média
    media_mes['media'] = media_mes['media'].round(2)

    # Fornecedores reprovados (com média abaixo de 70)
    reprov = media_mes[media_mes['media'] < 70]['fornecedor'].tolist()

    prompt = (
        f"Relatório técnico para o mês {mes} com IQF médio de {media_global}. "
    ...
    # Filtrar reprovados com média < 70
    reprovados_df = media_mes[media_mes['media'] < 70]
       # Criar lista com nome, nota e documento
    dados = [
        (
            row['fornecedor'],
            round(row['media'], 2),
            ''  # Se houver documento, é possível incluir aqui
        )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── PROCEDIMENTO ENGENMAN ─────────────────────────────────────────────
        ...
        meses_disponiveis = obter_meses_disponiveis()  # Função para obter meses disponíveis
        kb = [[InlineKeyboardButton(m, callback_data=f"mes_aprovados_{m.replace('/', '-')}")] for m in meses_disponiveis]
        return await update.callback_query.message.edit_text("📊 *Desempenho dos Fornecedores*\n\n"
           "Acompanhe o desempenho técnico dos fornecedores *aprovados*, conforme critérios do Procedimento PG.SM.02. Selecione o mês desejado para visualizar os resultados mensais.\n\n", reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")
    
    elif d == "atencao_mensal":
        meses_disponiveis = obter_meses_disponiveis()  # Função para obter meses disponíveis
        kb = [[InlineKeyboardButton(m, callback_data=f"mes_atencao_{m.replace('/', '-')}")] for m in meses_disponiveis]
        return await update.callback_query.message.edit_text("📊 *Desempenho dos Fornecedores*\n\n"
           "Acompanhe o desempenho técnico dos fornecedores em *atenção*, conforme critérios do Procedimento PG.SM.02. Selecione o mês desejado para visualizar os resultados mensais.\n\n", reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")
    
    elif d == "reprovados_mensal":
        meses_disponiveis = obter_meses_disponiveis()  # Função para obter meses disponíveis
        kb = [[InlineKeyboardButton(m, callback_data=f"mes_reprovados_{m.replace('/', '-')}")] for m in meses_disponiveis]
        return await update.callback_query.message.edit_text("📊 *Desempenho dos Fornecedores*\n\n"
           "Acompanhe o desempenho técnico dos fornecedores *reprovados*, conforme critérios do Procedimento PG.SM.02. Selecione o mês desejado para visualizar os resultados mensais.\n\n", reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")
    ...
    elif d.startswith("mes_aprovados_"):
        partes = d.split("_")
        mes = partes[2].replace("-", "/")
        page = int(partes[3]) if len(partes) == 4 else 0
        return await listar_aprovados_por_mes(update, context, mes, page)

    elif d.startswith("mes_reprovados_"):
        partes = d.split("_")
        mes = partes[2].replace("-", "/")
        page = int(partes[3]) if len(partes) == 4 else 0
        return await listar_reprovados_por_mes(update, context, mes, page)

    elif d.startswith("mes_atencao_"):
        partes = d.split("_")
        mes = partes[2].replace("-", "/")
        page = int(partes[3]) if len(partes) == 4 else 0
        return await listar_atencao_por_mes(update, context, mes, page)
    ...
    # SUPORTE E CONTATO DOS FORNECEDORES POR BASE ──────────────────────────────────────────────────────────
        ...
        df['CONTRATO'] = df['CONTRATO'].str.upper().str.strip()
        df['FORNECEDOR'] = df['FORNECEDOR'].str.strip()
        df['CONTRATO'] = df['CONTRATO'].str.extract(rf"({base})", expand=False)
        fornecedores = df[df['CONTRATO'].notna()].sort_values(by="FORNECEDOR").reset_index(drop=True)
...
async def listar_aprovados_individual(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    media_individual = (
        df.dropna(subset=['nota'])
          .groupby('nome_agente')['nota']
          .mean()
          .reset_index()
          .rename(columns={'nome_agente': 'fornecedor', 'nota': 'media'})
    )
    media_individual['media'] = media_individual['media'].round(2)

    fornecedores = media_individual[media_individual['media'] > 75].sort_values('fornecedor')
    nomes = list(fornecedores['fornecedor'])
...
async def listar_atencao_individual(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    fornecedores = media_individual[(media_individual['media'] >= 70) & (media_individual['media'] <= 75)].sort_values('fornecedor')
    nomes = list(fornecedores['fornecedor'])
...
async def listar_reprovados_individual(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    fornecedores = media_individual[media_individual['media'] < 70].sort_values('fornecedor')
    nomes = list(fornecedores['fornecedor'])
...
async def listar_aprovados_por_mes(update: Update, context: ContextTypes.DEFAULT_TYPE, mes: str, page: int = 0):
    ...
    df['nome_agente'] = df['nome_agente'].astype(str).str.strip().str.upper()
    df = df.dropna(subset=['nota', 'mes'])

    mes_formatado = pd.to_datetime(mes, format='%m/%Y').strftime('%m/%Y')
    df_mes = df[df['mes'] == mes_formatado]

    media_mes = (
        df_mes.groupby('nome_agente')['nota']
        .mean()
        .reset_index()
        .rename(columns={'nome_agente': 'fornecedor', 'nota': 'media'})
    )
    media_mes['media'] = media_mes['media'].round(2)
    fornecedores = media_mes[media_mes['media'] > 75].sort_values('fornecedor')
    ...
    if page < 0 or page >= total_paginas:
        return await update.callback_query.message.edit_text("❌ Página inválida.")

    botoes = [[InlineKeyboardButton(nome, callback_data=f"ai_{sha1(nome.encode()).hexdigest()}")] for nome in nomes[inicio:fim]]

    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Anterior", callback_data=f"mes_aprovados_{mes_formatado.replace('/', '-')}_{page-1}"))
    if fim < total:
        nav.append(InlineKeyboardButton("➡️ Próxima", callback_data=f"mes_aprovados_{mes_formatado.replace('/', '-')}_{page+1}"))
    if nav:
        botoes.append(nav)
    ...
    await update.callback_query.message.edit_text(
        f"✅ *Fornecedores Aprovados em {mes_formatado}* (Página {page+1}/{total_paginas})",
        reply_markup=InlineKeyboardMarkup(botoes),
        parse_mode="Markdown"
    )
async def listar_atencao_por_mes(update: Update, context: ContextTypes.DEFAULT_TYPE, mes: str, page: int = 0):
    df = carregar_dados_qualidade()
    df['nota'] = pd.to_numeric(df['nota'], errors='coerce')
    df['data'] = pd.to_datetime(df['data'], errors='coerce')
    df['mes'] = df['data'].dt.strftime('%m/%Y')
    ...
    media_mes['media'] = media_mes['media'].round(2)
    fornecedores = media_mes[(media_mes['media'] >= 70) & (media_mes['media'] <= 75)].sort_values('fornecedor')
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Anterior", callback_data=f"mes_atencao_{mes_formatado.replace('/', '-')}_{page-1}"))
    if fim < total:
        nav.append(InlineKeyboardButton("➡️ Próxima", callback_data=f"mes_atencao_{mes_formatado.replace('/', '-')}_{page+1}"))
    if nav:
        botoes.append(nav)

    botoes.append([InlineKeyboardButton("🔙 Voltar", callback_data="atencao_mensal")])

    for nome in nomes:
        fornecedor_id_map[f"ai_{sha1(nome.encode()).hexdigest()}"] = nome

    await update.callback_query.message.edit_text(
        f"⚠️ *Fornecedores em Atenção em {mes_formatado}* (Página {page+1}/{total_paginas})",
        reply_markup=InlineKeyboardMarkup(botoes),
        parse_mode="Markdown"
    )

async def listar_reprovados_por_mes(update: Update, context: ContextTypes.DEFAULT_TYPE, mes: str, page: int = 0):
    df = carregar_dados_qualidade()
    df['nota'] = pd.to_numeric(df['nota'], errors='coerce')
    df['data'] = pd.to_datetime(df['data'], errors='coerce')
    df['mes'] = df['data'].dt.strftime('%m/%Y')
    ...
    media_mes['media'] = media_mes['media'].round(2)
    fornecedores = media_mes[media_mes['media'] < 70].sort_values('fornecedor')
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Anterior", callback_data=f"mes_reprovados_{mes_formatado.replace('/', '-')}_{page-1}"))
    if fim < total:
        nav.append(InlineKeyboardButton("➡️ Próxima", callback_data=f"mes_reprovados_{mes_formatado.replace('/', '-')}_{page+1}"))
    if nav:
        botoes.append(nav)

    botoes.append([InlineKeyboardButton("🔙 Voltar", callback_data="reprovados_mensal")])

    for nome in nomes:
        fornecedor_id_map[f"ai_{sha1(nome.encode()).hexdigest()}"] = nome

    await update.callback_query.message.edit_text(
        f"❌ *Fornecedores Reprovados em {mes_formatado}* (Página {page+1}/{total_paginas})",
        reply_markup=InlineKeyboardMarkup(botoes),
        parse_mode="Markdown"
    )
...
async def listar_reprovados_mensal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    botoes = [
        [InlineKeyboardButton(m, callback_data=f"mes_reprovados_{m.replace('/', '-')}")]
    ...
    await update.callback_query.message.edit_text(
        "❌ *Fornecedores Reprovados*\n\nSelecione o mês desejado para visualizar os fornecedores reprovados (IQF abaixo de 70):",
        reply_markup=InlineKeyboardMarkup(botoes),
        parse_mode="Markdown"
    )
